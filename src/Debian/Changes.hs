...
changesFileName = render . pretty . PP

instance Pretty (PP ChangesFile) where
    pretty (PP changes) = text (changePackage changes ++ "_") <> prettyDebianVersion (changeVersion changes) <> text "_" <> prettyArch (changeArch changes) <> text ".changes"
...
instance Pretty (PP ChangeLogEntry) where
    pretty (PP (Entry package ver dists urgency details who date)) =
        hcat [ text package <> text " (" <> prettyDebianVersion ver <> text (") " <> intercalate " " (map codename dists) ++ "; urgency=" ++ urgency)
             , text "\n\n"
             , text "  " <> text (strip' details)
             , text "\n\n"
             , text (" -- " <> who <> "  " <> date)
             , text "\n" ]
    pretty (PP (WhiteSpace _)) = error "instance Pretty ChangeLogEntry"
...
instance Pretty (PP ChangeLog) where
    pretty (PP (ChangeLog xs)) = hcat (intersperse (text "\n") (map (pretty . PP) xs))
...
_showHeader (Entry package ver dists urgency _ _ _) =
    text (package <> " (") <> prettyDebianVersion ver <> text (") " <> intercalate " " (map codename dists) <> "; urgency=" <> urgency <> "...")
